...
class CompactarController:
    def __init__(self, mainWindow):
        ...
        self.mainWindow.tableFileC.itemClicked.connect(self.mostrar)
    ...
    def mostrar(self, item):
        ...
        nombre = self.mainWindow.tableFileC.item(fila, 0)    # Extraemos el objeto celda de la columna 0 (Nombre) en esa fila
    ...
    def comprimirArchivo(self):
        seleccion = self.mainWindow.tableFileC.selectedItems()
        if not seleccion:
            return
        fila = seleccion[0].row()
        nombreArchivo = self.mainWindow.tableFileC.item(fila, 0).text()
